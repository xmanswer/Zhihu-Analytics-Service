...
def get_user_keywords(uid, field, db, K=None, method=None):
    if K is None:
        K = defaultK
    if method is None:
        method = jieba.analyse.extract_tags
...
#K defines top K keywords, method can be extract_tags(tf-idf based) or textrank
def get_question_keywords(qid, db, K=None, method=None):
    if K is None:
        K = defaultK
    if method is None:
        method = jieba.analyse.extract_tags
...
#generate single user/question keywords
def generate_user_keywords(uid, db, K=None, method=None):
    if K is None:
        K = defaultK
    if method is None:
        method = jieba.analyse.extract_tags
    user_kw_wrapper((uid, db, K, method))

def generate_question_keywords(qid, db, K=None, method=None):
    if K is None:
        K = defaultK
    if method is None:
        method = jieba.analyse.extract_tags
    question_kw_wrapper((qid, db, K, method))


#generate all users/questions keywords in threads
def generate_users_keywords(db, threadpool = None, K=None, method=None):
    if K is None:
        K = defaultK
    if method is None:
        method = jieba.analyse.extract_tags
...
def generate_questions_keywords(db, threadpool = None, K=None, method=None):
    if K is None:
        K = defaultK
    if method is None:
        method = jieba.analyse.extract_tags
